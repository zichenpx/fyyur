...
class Show(db.Model):
  ...
  def __repr__(self):
    ...
    return 'Show(%s, %s)' % (self.venue_id, self.artist_id, self.start_time)
...
def search_artists():
  ...
  q = Artist.query.filter(Artist.name == search_term)
  response={
    "count": q.count(),
    "data": [{
      "id": q.id,
...
def shows():
  ...
  for show in shows:
    data.append({
      ...
      "start_time": str(show.start_time) # TypeError: Parser must be a string or character stream, not datetime
